            ...
            <tbody>
              {invoice.line_items?.map((item, index) => (
                <tr key={index} className="border-b">
                  ...
                  <td className="p-3 text-right">${formatCurrency(item.rate || item.unit_price || (item.total ? item.total / (item.quantity || 1) : item.amount / (item.quantity || 1)))}</td>
                  <td className="p-3 text-right font-medium">${formatCurrency(item.amount || item.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
