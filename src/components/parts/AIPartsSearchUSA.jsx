...
import { Sparkles, Search, Loader2, ExternalLink, DollarSign, Plus } from "lucide-react";
import { toast } from "sonner";
import { useCompany } from "../shared/CompanyContext";
import { useQueryClient } from "@tanstack/react-query";

export default function AIPartsSearchUSA() {
  ...
  const handleSearch = async () => {
    console.log("USA search button clicked");
  ...
  const handleAddToParts = async (source) => {
    ...
    try {
      const partData = {
        ...
        description: `Auto-imported from ${source.retailer} (USA)`,
        cost_price: source.price * 1.35,
        selling_price: source.price * 1.35 * 1.3,
        quantity: 0,
        reorder_level: 5,
      ...
      <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-200">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-blue-900">
            ...
            🇺🇸 USA-Wide Parts Search
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-gray-600 mb-4">
            Search across USA retailers (AutoZone, O'Reilly, Advance Auto, RockAuto)
          </p>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {isSearching ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-600">Searching USA retailers...</p>
            </div>
          ) : results ? (
            <div className="space-y-6">
              <div className="bg-blue-50 rounded-lg p-4">
                <h3 className="font-semibold text-blue-900 mb-2">Search: {results.part_searched}</h3>
                <p className="text-sm text-blue-700">{results.recommendation}</p>
              </div>

              {results.price_analysis && (
                <Card>
                  <CardHeader><CardTitle className="text-lg flex items-center gap-2"><DollarSign className="w-5 h-5 text-green-600" />Price Analysis (USD)</CardTitle></CardHeader>
                  <CardContent>
                    ...
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-400 mt-1">~${(source.price * 1.35)?.toFixed(2)} CAD</p>
                        </div>
                      </div>
