...
export default function AIVehicleSearchUSA() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await base44.integrations.Core.InvokeLLM({
        prompt: `Search for used vehicles for sale from major USA automotive marketplaces:
1. Cars.com
2. Autotrader.com
3. CarMax
4. Carvana
...
Provide comparison and recommendations across marketplaces.`,
      ...
      toast.error("Failed to search USA marketplaces. Please try again.");
      setDialogOpen(false);
    } finally {
  ...
  const handleAddToInventory = async (listing) => {
    ...
    try {
      const vehicleData = {
        ...
        vin: `US-${Date.now()}`,
        mileage: Math.round((listing.mileage || 0) * 1.609),
        condition: listing.condition?.toLowerCase() || "used",
        status: "in_stock",
        purchase_price: Math.round((listing.price || 0) * 1.35),
        selling_price: Math.round((listing.price || 0) * 1.35 * 1.15),
        location: listing.location || "USA",
        notes: `Source: ${listing.marketplace} (USA). ${listing.features || ''}`,
      ...
      <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-200">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-blue-900">
            ...
            🇺🇸 USA-Wide Vehicle Search
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-gray-600 mb-4">
            Search Cars.com, Autotrader, CarMax, Carvana, and other USA marketplaces
          </p>
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2">
              ...
              <Input placeholder="Max Price (USD)" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="text-sm" />
            </div>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {isSearching ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-600">Searching USA marketplaces...</p>
            </div>
          ) : results ? (
            <div className="space-y-6">
              ...
              {results.price_analysis && (
                <Card>
                  <CardHeader><CardTitle className="text-lg">Price Analysis (USD)</CardTitle></CardHeader>
                  <CardContent>
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <Badge className="bg-blue-100 text-blue-800">{listing.marketplace}</Badge>
                          </div>
                          {listing.mileage > 0 && <p className="text-sm text-gray-600">📏 {listing.mileage.toLocaleString()} miles (~{Math.round(listing.mileage * 1.609).toLocaleString()} km)</p>}
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-400 mt-1">~${Math.round((listing.price || 0) * 1.35).toLocaleString()} CAD</p>
                        </div>
                      </div>
